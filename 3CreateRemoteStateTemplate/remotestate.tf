...
resource "aws_iam_group_membership" "add-ec2admin" {
    name = "add-sally"
    users = [ aws_iam_user.sallysue.name, aws_iam_user.marymoe.name, ]
...
resource "aws_iam_group_membership" "add-rdsadmin" {
  name = "add-rdsadmin"
  users = [ aws_iam_user.sallysue.name,]
...
##################################################################################
# Create IAM Account and Access key and aws_iam_user_policy
##################################################################################
...
EOF
    filename = "/home/zhanga/.aws/credentials"
